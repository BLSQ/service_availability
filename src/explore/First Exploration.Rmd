...
data_snis_full <- read.csv('data/raw/snis_penta_data.csv')
...
```{r}
library(zoo)
data_rupture$month <- as.factor(as.yearmon(paste0(as.character(data_rupture$period), '15'), format="%Y%m%d"))
data_test <- data_rupture[data_rupture$zone %in% unique(data_rupture$zone)[0:20],]
d <- ggplot(data_test, aes(month, org_unit_ID, fill = value))
d + geom_bin2d( ) + #+ scale_x_date(date_breaks = "1 month", date_labels = "%m")+ 
  scale_fill_gradient(name = "Mean reported stockout days",
                      high = '#ef0404', low = '#6ddb3f',
                       space = "Lab",
                      na.value = "grey50", guide = FALSE) +#+ +
  theme( axis.text.y=element_blank(),
         axis.ticks.y=element_blank()) + facet_wrap(~zone)
